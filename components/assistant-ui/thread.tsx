...
export const Thread: FC<ComponentProps<typeof ThreadPrimitive.Root> & { welcomeMessage?: string }> = ({ welcomeMessage, ...props }) => {
  return (
...
const ThreadWelcome: FC<{ message?: string }> = ({ message }) => {
  return (
...
const AssistantActionBar: FC = () => {
  const useMessageResult = useMessage();
  // Assuming useMessage returns an object, but if it returns the message object directly,
  // we need to be careful. Let's assume it returns the context.
  // If useMessage is typed as returning ThreadMessage, it might not have isCopied directly,
  // or isCopied might be state.
  // Actually, based on older docs, it might be useMessageStore or similar.
  // But let's assume standard behavior for now.
  // However, since I cannot verify the type, I will cast to any to be safe for now,
  // or check if properties exist.
  // Wait, isCopied is likely local UI state.
  // Assistant UI handles copy state internally?
  // Let's check ActionBarPrimitive.Copy
  
  // Actually, let's just use the Copy primitive's built-in state if possible, 
  // but here we need to switch icon.
  // Usually ActionBarPrimitive.Copy provides state via render props or context?
  // No, the example used AssistantIf condition={({ message }) => message.isCopied}.
  // This implies the message object in context has an isCopied property.
    ...
    >
      <ActionBarPrimitive.Copy asChild>
        <TooltipIconButton tooltip="Copy">
          {message.isCopied ? <CheckIcon /> : <CopyIcon />}
        </TooltipIconButton>
